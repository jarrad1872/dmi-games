/**
 * Obstacle - Polished construction site obstacles
 * Clean geometric designs with visual variety
 */
...
export class Obstacle {
  ...
  private renderBarrier(ctx: CanvasRenderingContext2D, bounds: { x: number; y: number; width: number; height: number }): void {
    // Concrete barrier with orange/white stripes
  ...
  private renderScaffold(ctx: CanvasRenderingContext2D, bounds: { x: number; y: number; width: number; height: number }): void {
    ...
    ctx.fillRect(bounds.x - 5, bounds.y + bounds.height / 2 - 3, bounds.width + 10, 6);
    ctx.fillStyle = '#000';
    for (let i = 0; i < bounds.width + 10; i += 12) {
      ctx.fillRect(bounds.x - 5 + i, bounds.y + bounds.height / 2 - 3, 6, 6);
    }
  }
  ...
  private renderForklift(ctx: CanvasRenderingContext2D, bounds: { x: number; y: number; width: number; height: number }): void {
    ...
    if (flash) {
      ...
      ctx.beginPath();
      ctx.arc(bounds.x + bounds.width / 2, bounds.y + 5, 6, 0, Math.PI * 2);
      ctx.fill();
      
      // Glow
      ctx.fillStyle = 'rgba(255, 69, 0, 0.3)';
      ctx.beginPath();
      ctx.arc(bounds.x + bounds.width / 2, bounds.y + 5, 15, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ...
  private renderRebar(ctx: CanvasRenderingContext2D, bounds: { x: number; y: number; width: number; height: number }): void {
    ...
    const rebarColor = '#8B4513';
    const rustColor = '#A0522D';
